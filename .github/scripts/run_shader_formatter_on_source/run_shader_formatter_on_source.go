...
func main() {
	...
	var path_to_shader_formatter = os.TempDir()
	setup_shader_formatter(path_to_shader_formatter)
	path_to_shader_formatter += "/shader-formatter"
	...
	err = session.Command(path_to_shader_formatter, path_to_shaders, "--only-scan").Run()
	if err != nil {
		fmt.Println(err)
		os.Exit(1)
	}
