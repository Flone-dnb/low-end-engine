...
LongLongInspector::LongLongInspector(
    ...
    const auto variableIt = typeInfo.reflectedVariables.longLongs.find(sVariableName);
    if (variableIt == typeInfo.reflectedVariables.longLongs.end()) [[unlikely]] {
    ...
    setSize(glm::vec2(getSize().x, 0.05F));
    {
        ...
        pBackground->setColor(EditorTheme::getButtonColor());
        {
            ...
            pTextEdit->setOnTextChanged([this, pTextEdit](std::u16string_view sNewText) {
                ...
                const auto variableIt = typeInfo.reflectedVariables.longLongs.find(this->sVariableName);
                if (variableIt == typeInfo.reflectedVariables.longLongs.end()) [[unlikely]] {
                    Error::showErrorAndThrowException(
