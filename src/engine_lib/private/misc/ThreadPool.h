...
public:
     ...
     * Can be called explicitly. If not called explicitly will be called in destructor.
     */
    void stop();

    /**
     * Tells if @ref stop was called or not.
     *
     * @return State.
     */
    bool isStopped() const { return bIsShuttingDown.test(); }
...
private:
    ...
    const unsigned int iMinThreadCount = 2;
};
