...
public:
     ...
     */
    [[nodiscard]] static std::optional<Error> removeFile(ConfigCategory category, std::string_view sFileName);
     ...
     */
    template <typename T> T getValue(std::string_view sSection, std::string_view sKey, T defaultValue) const;
     ...
     * @return Error if something went wrong.
     */
    [[nodiscard]] std::optional<Error> saveFile(std::filesystem::path pathToConfigFile, bool bEnableBackup);
...
private:
     ...
     */
    static std::string
    generateFreeFileName(const std::set<std::string>& usedFileNames, const std::string& sFileNamePrefix = "");
